                ...
                <div key={post.post.id} className="mt-4 p-4 border rounded-md border-dashed border-slate-400">
                    <div className="mb-2 flex items-start justify-between">
                        <div>
                            ...
                            <small className="text-xs text-slate-600">Create by {post.user.name} on {new Date(post.post.created_at).toLocaleTimeString()}</small>
                        </div>
                    </div>
                    <p>{post.post.body}</p>
                    ...
                    {user && user.id === post.post.user_id &&
                        <div className="flex items-center justify-end gap-4">
                            <Link to={`/posts/update/${post.post.id}`} className="bg-green-500 text-white text-sm rounded-lg  px-5 py-1">Update</Link>
